...
const api = async (method, path, body) => {
  try {
    ...
    return await (await fetch(url(`api${path}`), opts)).json();
  } catch (e) {
...
async function init() {
  try {
    const aboutRes = await fetch(url("api/about"));
...
<cap-widget data-cap-api-endpoint="${endpoint}"></cap-widget>`;
  const nodeSnippet = `const res = await fetch("${origin}/api/v0/siteverify", {
  method: "POST",
