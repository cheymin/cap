...
import { randomBytesHex } from "./_crypto.js";
...
// Cloudflare Workers: CF-Connecting-IP 优先
...
async function isBlocked(siteKey, ip) {
  ...
  for (const [key, val] of entries) {
    ...
    if (key.startsWith("asn:")) continue;
...
capApp.post("/:siteKey/challenge", capRateLimiter, async (c) => {
  ...
  (async () => {
    ...
    // Cloudflare 特有：CF-IPCountry / CF-IPCity 头
    if (!country) {
...
capApp.post("/:siteKey/redeem", capRateLimiter, async (c) => {
  ...
  const result = await coreValidateChallenge(
    ...
    },
    {
      ...
      signToken: () => {
        const redeemId = randomBytesHex(8);
        const redeemSecret = randomBytesHex(15);
