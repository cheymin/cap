...
import { hashPassword, randomBytesHex, randomBytesBase64 } from "./_crypto.js";
...
serverApp.post("/keys", async (c) => {
  ...
  const secretKey = `sk-${randomBytesBase64(32).replace(/\+/g, "").replace(/\//g, "").replace(/=+$/, "")}`;
  const jwtSecret = randomBytesBase64(32).replace(/\+/g, "").replace(/\//g, "").replace(/=+$/, "");
...
serverApp.post("/keys/:siteKey/rotate-secret", async (c) => {
  ...
  const newSecretKey = `sk-${randomBytesBase64(32).replace(/\+/g, "").replace(/\//g, "").replace(/=+$/, "")}`;
...
serverApp.get("/settings/sessions", async (c) => {
  ...
  return c.json(sessions.map((s) => {
    ...
    return {
      ...
      expires: new Date(Number(data.expires || s.expires)).toISOString(),
      created: new Date(Number(data.created || s.created)).toISOString(),
    };
  }));
});
...
serverApp.post("/settings/apikeys", async (c) => {
  ...
  const token = randomBytesBase64(32).replace(/\+/g, "").replace(/\//g, "").replace(/=+$/, "");
